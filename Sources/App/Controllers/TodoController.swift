...
final class TodoController {
    ...
    func index(_ req: Request) throws -> Future<[Todo.Outgoing]> {
        return Todo.query(on: req).all().map { todos in
            return try todos.map { try $0.makeOutgoing(with: req) }
        }
    }
    ...
    func create(_ req: Request) throws -> Future<Todo.Outgoing> {
        return try req.content.decode(Todo.Incoming.self).flatMap { todoIncoming in
            return todoIncoming.makeTodo().save(on: req)
